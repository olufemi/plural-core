...
- P2P FX operations bridge: `controller/BoP2pFxOpsController.java`
- VAS/catalog/airtime reversal support: `controller/BoFxPeerServicesController.java`
...
| Admin profile/self-service | Password change/recovery, MFA setup, `/bo/admin-users/me`, `/bo/admin-users/me/activity`, and admin list rows with timestamps/login/security fields exist. | Partial / stronger pilot foundation | Need notification preferences, trusted devices, profile update controls beyond fullName, and self-service MFA reset policy. |
...
| P2P FX Exchange Admin | Typed offer update endpoint exists with mandatory reason. New `/backoffice/p2p-fx` bridge exposes existing downstream market offer browse, seller offer oversight, rate/cancel intervention, escrow init/detail/fund/release, seller ratings/stats, and receipt HTML. | Partial / stronger bridge foundation | Still missing full trade lifecycle dashboard, fulfilled-trade views, spread/corridor controls, dispute case management, fraud/AML flags, settlement/reconciliation, maker-checker policy decisions for escrow operations, and FX reports. |
| Global VAS / Sochitel Admin | Airtime reversal summary/cases/retry exist. New `/backoffice/fxpeer/services` wrappers expose featured services, categories, product lookup, products by category, and products by country. | Partial / stronger bridge foundation | Still missing VAS order dashboard, transaction detail, provider reconciliation, duplicate detection, refund workflow, Sochitel request/response log search, provider health/SLA, pricing/margin governance, and full VAS reporting. |
| Finance and Treasury | Reversal exception cases, manual reversal approval path, and typed interbank name-enquiry admin utility exist. | Minimal / safer admin-tool foundation | Missing wallet ledger views, GL posting visibility, settlement files, bank/custodian reconciliation, fee/revenue reporting, treasury balances, payout controls, chargeback/refund center, and financial close reports. |
...
- P2P FX: FE can now start with `/backoffice/p2p-fx/market/offers`, `/seller-offers`, escrow detail/actions, seller stats/ratings, and receipt views. Mutating offer/escrow actions require a `reason`, and escrow fund actions require `Idempotency-Key`.
- VAS: FE can now start with `/backoffice/fxpeer/services/featured`, `/categories`, `/products`, `/products/by-category`, `/products/by-country`, and airtime reversal summary/list/retry.
