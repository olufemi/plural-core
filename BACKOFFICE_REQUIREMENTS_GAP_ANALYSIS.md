...
| Investment Admin | Product list/detail workaround, create/update product, featured services config, liquidation approve/deny, orders, liquidations, history, performance, dashboard, oversight, CSV export. | Strong partial | Need consistent product detail API, validation hardening, maker-checker for product/rate changes, rate/yield calendar, maturity/rollover controls, custodian settlement, NAV/unit pricing history, risk limits, portfolio exposure views, bulk exports, and reconciliation reports. |
...
| Customer Management | Customer list/detail, investment summaries/orders/liquidations/positions, block/unblock with optional maker-checker policy. | Partial | Missing full customer 360: KYC docs/status, BVN/face verification status, wallets/accounts, transaction history, support notes, linked devices, risk flags, service enrollments, referral history, communication log, view-only impersonation, and re-verification workflows. |
...
- Customer: customer 360 baseline with profile, wallets/accounts, devices, KYC/verification status, transactions summary, investment exposure. Block/unblock now has optional maker-checker policy support.
