...
| 3.1 page 4 | RBAC-07 approval rights by module/submodule | Strong Pilot Foundation | Approval permissions exist and checker endpoints accept liquidation, reversal, product config, app_config, referral/campaign, and customer profile approval authorities | Good pilot foundation for selected workflows, with same-maker/same-checker prevention still enforced by the approval service | Extend to rates, wallet/treasury, VAS, finance, and admin-user high-risk changes |
...
| 3.2 page 5 | MC-02 pending queue | Implemented / Partial | `ApprovalController` list/detail/approve/reject endpoints | Queue exists for current approval types and checker permissions cover currently wired approval domains | Broaden eligible checker filtering by priority/SLA and future modules |
...
| 3.3 page 5 | AUTH-01 mandatory MFA | Strong Pilot Foundation | `MfaController`, login MFA verify flow, `BO_MFA_REQUIRED` flag | MFA capability exists and can be made mandatory by environment config | Enable `BO_MFA_REQUIRED=true` in prod only after super-admin MFA setup and recovery runbook are confirmed |
| 3.3 page 5 | AUTH-03 configurable session timeout | Strong Pilot Foundation | Auth refresh/logout/session endpoints, rotating refresh tokens, `BO_IDLE_TIMEOUT_SECONDS`, `BO_PRIVILEGED_IDLE_TIMEOUT_SECONDS`, `BO_EXPIRY_WARNING_SECONDS` | Backend exposes session policy to FE and supports session list/revoke/revoke-all | FE must enforce idle UX; add server-side request idle enforcement if required by final security signoff |
...
| 3.4 page 6 | AUD-01 audit every sensitive/config action with actor, role, timestamp, IP/device, before/after | Stronger Pilot Foundation | `AuditAspect`, `AdminAuditLog`, `BoAuditLog`, app config history | Audit foundation now captures request method, URI, IP, user-agent, outcome, and failure class for audited endpoints; app_config/product approvals preserve before/after where wired | Verify before/after snapshots on every high-risk endpoint and define immutable archive retention |
...
| 3.6 page 6 | Reporting analytics framework | Partial / Stronger Pilot Foundation | `ReportController`, `CsvWriter`, investment CSV export | Report catalog exposes formats/permissions, jobs echo filters/parameters, schedules can be created/listed/deleted, and CSV/export path exists | Persist report jobs/schedules for prod, then add XLSX/PDF where required |
...
| 5.1 page 10 | INV-01 to INV-12 investment product configuration | Strong Pilot Foundation | `BoInvestmentController` product list/detail/create/update/history, `InvestmentProductUpsertRequest`, FxPeer direct product lookup, `ApprovalService` product approvals | Product admin exists, mandatory field validation was improved, direct product detail/history endpoints avoid brittle list-search 404s, and create/update now require maker-checker with before/after snapshots in approval history | Need deeper product validation coverage, deletion/deactivation rules, and production-grade immutable audit retention |
...
| 5.3 page 13 | Rate, yield, unit price management | Partial / External | Product fields, product detail/history metadata, approval before/after snapshots, and performance endpoint exist | Current rate/product values and approved product config changes are visible | Need dedicated rate/NAV scheduling, effective-date approval, and reconciliation |
...
| 6 pages 19-20 | P2P FX listing, escrow, lifecycle, fraud, reporting | Partial / stronger bridge foundation | `BoFxPeerOfferController`, `BoP2pFxOpsController`, `FxPeerExchangeClient`; bridges market browse, seller offer oversight, rate/cancel intervention, escrow init/detail/fund/release, seller ratings/stats, and receipt HTML | FE now has secured backoffice endpoints for the existing FXPeer offer/market/escrow/rating/receipt primitives | Still need full trade lifecycle dashboard, fulfilled-trade views, dispute case management, AML/fraud flags, corridor controls, settlement/reconciliation workspace, maker-checker policy decisions for escrow actions, and FX reports |
| 7 pages 21-22 | Global VAS / Sochitel order management, retry, refund, reconciliation, reporting | Partial / stronger bridge foundation | `BoFxPeerServicesController`, `FxPeerExchangeClient`; bridges featured services, categories, product lookup, products by category/country, reversal summary/list/retry | FE now has provider/catalog visibility and reversal retry support through backoffice | Still need order dashboard, transaction detail, provider reconciliation, duplicate detection, refund workflow, Sochitel request/response log search, provider health/SLA, and VAS reports |
...
| 9 pages 23-25 | Customer directory, search, 360, wallet/transaction visibility, enrollment, support actions | Partial / Stronger Pilot Foundation | `ProfilingManagementController` customer list/detail, `/backoffice/profiling/{id}/customer-360`, investment views, block/unblock with optional maker-checker policy | Customer lookup, profile, KYC/BVN status summary, primary wallet, additional accounts, devices, referral context, access status, basic timeline, investment sections, and selected governed actions exist | Add wallet ledger/balance source, full transaction history, support notes, communication log, risk flags, service enrollments, full KYC document archive, and re-verification workflow |
...
3. Add maker-checker policy table/config and enforce it first on product config, referral campaign changes, customer block/unblock, `app_config`, reversals, and liquidations.
   ...
   - 2026-07-16 progress: widened checker authorization so product/app_config/referral/campaign/customer approvals can be approved/rejected by their matching permissions, while the approval service still prevents same-user maker/checker decisions.
   - 2026-07-31 progress: redemption approval mode, auto-approval threshold, scheduler switch, and scheduler cron are now governed through backoffice `app_config` rows with maker-checker registration. Dev may fall back to FxPeer AUTO behavior, while pilot/prod must explicitly approve the mode/threshold before go/no-go.
...
5. Harden admin session/audit/reporting APIs for pilot-to-prod.
...
8. Expand P2P FX and VAS backoffice bridges from available downstream contracts.
   - 2026-07-16 progress: added `/backoffice/p2p-fx` secured bridges for FX market offers, seller offers, offer rate/cancel interventions, escrow init/detail/fund/release, seller ratings/stats, and receipt HTML. Added VAS category/product lookup wrappers and airtime reversal retry alias under `/backoffice/fxpeer/services`. Updated FE API reference.
9. Harden investment product create/update validation and make FE error responses deterministic.
10. Expand customer 360 with KYC/BVN/face status, wallets/accounts, devices, and service enrollment views.
11. Decide whether deeper VAS and P2P FX operations are pilot or production-later; if pilot, build operational dashboards before go-live.
