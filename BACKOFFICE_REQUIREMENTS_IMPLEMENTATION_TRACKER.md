...
| 5.1 page 10 | INV-01 to INV-12 investment product configuration | Partial / Strong Pilot Foundation | `BoInvestmentController` product list/detail/create/update/history, `InvestmentProductUpsertRequest`, FxPeer direct product lookup | Product admin exists, mandatory field validation was improved, and direct product detail/history endpoints now avoid brittle list-search 404s | Need maker-checker, before/after config audit, rate/yield history, deletion/deactivation rules, full validation coverage |
...
| 5.3 page 13 | Rate, yield, unit price management | Partial / External | Product fields, product detail/history metadata, and performance endpoint exist | Current rate/product values and lifecycle timestamps are visible | Need rate/NAV scheduling, approval, before/after history, and reconciliation |
